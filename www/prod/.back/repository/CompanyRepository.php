...
class CompanyRepository
{
    ...
    {
        return "SELECT HEX(id) as id, name, description, siren, is_active, HEX(sector_id) as sector_id FROM company";
    }
    ...
    public function push(CompanyModel $company): bool
    {
        try {
            ...
            $stmt->execute([
                ...
                'siren' => $company->siren,
                'email' => $company->email->asString() ?? null, // Gestion du null si optionnel
    ...
    public function search(array $filters): array
    {
        ...
        // Ensure 'page' exists and is at least 1 to avoid negative offsets
        $page = max(1, (int) ($filters['page'] ?? 1));
        $offset = ($page - 1) * $limit;

        // 1. Updated table name to 'company' (singular) to match your push() method
        // 2. Added HEX() conversion for IDs if they are stored as BINARY in DB
        ...
        if (!empty($filters['name'])) {
            $sql .= " AND (c.name LIKE :name_search OR c.siren LIKE :name_search)";
            $params['name_search'] = '%' . $filters['name'] . '%';
        }

        // Filter by Sector (Handling HEX input)
        ...
        if (isset($filters['status']) && $filters['status'] !== '') {
            ...
            $params['is_active'] = ($filters['status'] === 'active' || $filters['status'] === '1') ? 1 : 0;
        }
