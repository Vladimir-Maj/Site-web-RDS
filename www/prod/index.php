...
$router->add('GET',  '/dashboard/companies/' . $idPattern, fn($p, $pdo, $twig) => $compHandler($pdo, $twig)->renderForm($p[0]), roles: $staff);
$router->add('POST', '/dashboard/companies/' . $idPattern, fn($p, $pdo, $twig) => $compHandler($pdo, $twig)->handleFormSave($p[0]), roles: $staff);
...
$router->add('GET', '/app/offers/show/' . $idPattern,     fn($p, $pdo, $twig) => $offerHandler($pdo, $twig)->show($p[0]));
...
$router->add('POST', '/app/offers/delete/' . $idPattern,  fn($p, $pdo, $twig) => $offerHandler($pdo, $twig)->destroy($p[0]), roles: $staff);

// ── SITES ────────────────────────────────────────────────────────────────────
$router->add('GET',  '/dashboard/companies/' . $idPattern . '/sites', fn($p, $pdo, $twig) => $siteHandler($pdo, $twig)->index($p[0]), roles: $staff);
...
$router->add('GET',  '/app/sites/' . $idPattern,                     fn($p, $pdo, $twig) => $siteHandler($pdo, $twig)->show($p[0]), roles: $staff);
$router->add('POST', '/app/sites/delete/' . $idPattern,              fn($p, $pdo, $twig) => $siteHandler($pdo, $twig)->delete($p[0]), roles: $staff);
...
$router->add('PATCH',  '/api/skills/update/' . $idPattern,    fn($p, $pdo, $twig) => $skillHandler($pdo, $twig)->updateAjax($p[0]), roles: $staff);
$router->add('DELETE', '/api/skills/delete/' . $idPattern,    fn($p, $pdo, $twig) => $skillHandler($pdo, $twig)->deleteAjax($p[0]), roles: $staff);
